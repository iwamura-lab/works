...
class ProjMatrix():
    ...
    def calc_rep(self):
        ...
        if self.sph_basis is None:
            self.rep_matrix = calc_ghm_kron_rep(self.exponents)
        ...
        # else, use both of them
            ...
            ghm_matrix = calc_ghm_kron_rep(self.exponents)
            self.rep_matrix = np.kron(sph_matrix, ghm_matrix)

    def calc_number_of_invs(self):
        '''
        Calclulate the number of invariants.
        If there is no invarinats, there is no need to calc matrix
        '''
        matrix = self.rep_matrix
        chi = np.trace(matrix)
        num_of_inv = integrate_element(chi)
        print('The Raw Number Of Invariants Is {}'.format(str(num_of_inv)))

        # Cast to int
        num = int(np.real(np.round(num_of_inv)))
        print('The Number Of invariants Is {}'.format(str(num)))

        if num == 0:
            print('There is no need to calc matrix')
            sys.exit()
        else:
            pass
    ...
    def calc_invs(self):
        ...
        # Cast to real value
        # real_mat = np.real(matrix)
        com_mat = np.imag(matrix)
        _val = np.max(com_mat)
        print('The Max Complex Value Of Matrix Is {}'.format(str(_val)))
    ...
    def _measure_calc_time(self):
        '''
        Measure the calc time of projection
        '''
        import time
        import random
        ...
        # Start calc one element of matrix
        # Choose it at random
        size = self.rep_matrix.shape[0]
        row = random.randrange(0, size)
        col = random.randrange(0, size)
        ele = self.rep_matrix[row, col]
        _ = integrate_element(ele)

        sec_elapse_time = time.time() - start
        # Convert to minutes
        minutes_time = sec_elapse_time/60

        print('The one calc time is : {}minutes'.format(str(minutes_time)))
...
def calc_each_sph_rep(l):
    ...
    for i, m in enumerate(range(-l, l + 1)):
        vec = [WignerD(l, m, mp, 0, b, 0) * (exp(-m * a * I))
               * (exp(-mp * c * I)) for mp in range(-l, l + 1)]
...
def calc_ghm_kron_rep(exp_lst):
    ...
    ----------------------------------------------------------
    '''
    _reps = [calc_each_ghm_rep(exp) for exp in exp_lst]
    for i, arr in enumerate(_reps):
        if i == 0:
            matrix = arr
            new_matrix = arr
...
def calc_proj_matrix(matrix):
    ...
    print('Calculation will be done for ' + str(len(_uniq_lst)))
...
def integrate_element(element, i=None):
    ...
    # Integrate b value
    # If rish isnt set to False, rasie error
    # I dont know the exact reason
    val3 = integrate(val2 * sin(b), (b, 0, math.pi)).evalf()
    ...
    if result is np.nan:
        ...
        print(val2)
        print(val3)
        raise Exception('Nan value apperrs!!')
    ...
    # Define variables
    a, b, c = symbols('a b c')
    # Integrate about a value
    _tmp1 = integrate(element, (a, 0, 2 * np.pi))
    # Simplify
    _tmp1 = simplify(_tmp1)
    # _tmp1 = nsimplify(_tmp1)

    # Integrate about c value
    _tmp2 = integrate(_tmp1, (c, 0, 2 * np.pi))
    _tmp2 = simplify(_tmp2)
    # _tmp2 = nsimplify(_tmp2)
    # _tmp2 = _tmp2.doit()
    # _tmp2 = simplify(_tmp2)
    print(_tmp2)
    ...
    # Use scipy quad method, because sympy cannot integrate this functino
    _real, _imag = _tmp2.as_real_imag()
    # Take product which sin function
    real, imag = _real * sin(b), _imag * sin(b)

    def __real_func(b_val):
        res = real.subs(b, b_val).doit()
        return res

    def __imag_func(b_val):
        res = imag.subs(b, b_val).doit()
        return res

    real_val = quad(__real_func, 0, np.pi)[0]
    complex_val = quad(__imag_func, 0, np.pi)[0]

    result = np.complex(real_val + complex_val * I)

    '''a, b, c = symbols('a b c')

    # Convert expression to familar style
    pol = element.doit()
    _real, _imag = pol.as_real_imag()
    real, imag = _real * sin(b), _imag * sin(b)
    '''

    '''
    def __real_func(a_val, b_val, c_val):
        res = real.subs([(a, a_val), (b, b_val), (c, c_val)])
        return res

    def __imag_func(a_val, b_val, c_val):
        res = imag.subs([(a, a_val), (b, b_val), (c, c_val)])
        return res

    real_val = tplquad(__real_func, 0, 2 * math.pi, lambda c: 0,
                       lambda c: math.pi, lambda c, b: 0,
                       lambda c, b: 2 * math.pi)[0]

    imag_val = tplquad(__imag_func, 0, 2 * math.pi, lambda c: 0,
                       lambda c: math.pi, lambda c, b: 0,
                       lambda c, b: 2 * math.pi)[0]

    result = np.complex(real_val + imag_val * I)
    '''
    if i is None:
        return result
    else:
...
if __name__ == '__main__':
    INS = ProjMatrix()
    INS.set_basis(None, [2])
    INS.calc_rep()
    INS._measure_calc_time()
    INS.calc_number_of_invs()
    INS.projection()
